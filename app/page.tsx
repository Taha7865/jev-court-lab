  ...
  return <main>
    ...
    <div className="workspace">
      ...
      <aside className="analysis-column" aria-label="Jev analysis">
        ...
        <div className="pass-result"><span>{result?label(result.choice).toLowerCase().replace(/^./,c=>c.toUpperCase()):'Best action'}</span><strong>{result?`${Math.round(result.probabilities[result.choice]*100)}%`:'—'}</strong></div>
        <p className="small">Jev’s preference for the top action, not the chance of scoring. {passProbability!==null&&`Either pass direction combined: ${Math.round(passProbability*100)}%.`}</p>
        ...
        <details className="extras"><summary>How Jev chooses an action</summary><p><strong>Shoot:</strong> Finish an available layup, dunk or close shot, or take a good open shot in range. A player moving toward the hoop can still be ready to shoot.</p><p><strong>Drive:</strong> Prioritize only a supported one-on-one opportunity: one close defender, room beyond them, and no second defender or help blocking the route. A good available shot or better pass still wins. If the matchup or route is unclear, do not make driving the first choice. Being at the top of the key alone is not a reason to drive.</p><p><strong>Pass:</strong> Find a teammate with a better opportunity or escape pressure. Do not give up a good shot just because another teammate is also open. Left and right refer to the screen.</p><p><strong>Reset:</strong> Keep possession when the shot, drive and passing options offer no clear advantage.</p><p>Jev receives detected geometry and earlier observations. Court zones, true shooting range, shot readiness and defender reach are not measured. An apparent opening can be misleading because of perspective or missed detections.</p></details>
        ...
        <details className="extras"><summary>Decision history ({events.length})</summary><div className="events">{events.map(e=><button key={e.id} className="event" onClick={()=>openEvent(e)}><time>{stamp(e.state.time_seconds)}</time><span>{label(e.decision.choice).toLowerCase().replace(/^./,c=>c.toUpperCase())} {Math.round(e.decision.probabilities[e.decision.choice]*100)}%</span></button>)}</div>{currentEvent&&<label>Observed action<select value={currentEvent.observed??''} onChange={e=>setEvents(a=>a.map(v=>v.id===currentEvent.id?{...v,observed:e.target.value as Action}:v))}><option value="" disabled>Select action</option>{ACTIONS.map(a=><option key={a} value={a}>{label(a)}</option>)}</select></label>}</details>
      </aside>
    </div>
